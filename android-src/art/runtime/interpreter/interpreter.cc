...
static void InterpreterJni(Thread *self, ArtMethod *method, const StringPiece &shorty,
    ...
    // TODO: The following enters JNI code using a typedef-ed function rather
    // than the JNI compiler,
    ...
    if (method->IsStatic()) {
        if (shorty == "L") {
            ...
            ScopedLocalRef<jclass> klass(
                soa.Env(), soa.AddLocalReference<jclass>(method->GetDeclaringClass()));
            jobject jresult;
            {
            ...
            ScopedLocalRef<jclass> klass(
                soa.Env(), soa.AddLocalReference<jclass>(method->GetDeclaringClass()));
            ScopedLocalRef<jobject> arg0(
                soa.Env(), soa.AddLocalReference<jobject>(reinterpret_cast<Object *>(args[0])));
            jobject jresult;
            {
                ScopedThreadStateChange tsc(self, kNative);
            ...
            ScopedLocalRef<jobject> arg0(
                soa.Env(), soa.AddLocalReference<jobject>(reinterpret_cast<Object *>(args[0])));
            ScopedLocalRef<jobject> arg1(
                soa.Env(), soa.AddLocalReference<jobject>(reinterpret_cast<Object *>(args[1])));
            ScopedThreadStateChange tsc(self, kNative);
            ...
            ScopedLocalRef<jobject> arg1(
                soa.Env(), soa.AddLocalReference<jobject>(reinterpret_cast<Object *>(args[1])));
            ScopedLocalRef<jobject> arg2(
                soa.Env(), soa.AddLocalReference<jobject>(reinterpret_cast<Object *>(args[2])));
            ScopedThreadStateChange tsc(self, kNative);
...
static JValue Execute(Thread *self, const DexFile::CodeItem *code_item, ShadowFrame &shadow_frame,
                      JValue result_register) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_);
...
static inline JValue Execute(Thread *self, const DexFile::CodeItem *code_item,
    ...
    //
    // http://androidxref.com/6.0.1_r10/xref/art/compiler/driver/compiler_driver.cc#2278
    // ```java
    ...
    //
    //
    // <init> is the (or one of the) constructor(s) for the instance, and
    // non-static field initialization.
    // <clinit> are the static initialization blocks for the class, and static
    ...
    if (LIKELY(shadow_frame.GetMethod()->IsPreverified())) {
        ...
        if (kInterpreterImplKind == kSwitchImpl) {
            if (transaction_active) {
                return ExecuteSwitchImpl<false, true>(self, code_item, shadow_frame,
                                                      result_register);
            } else {
        ...
        if (kInterpreterImplKind == kSwitchImpl) {
            if (transaction_active) {
                return ExecuteSwitchImpl<true, true>(self, code_item, shadow_frame,
                                                     result_register);
            } else {
                return ExecuteSwitchImpl<true, false>(self, code_item, shadow_frame,
                                                      result_register);
            }
        } else {
            DCHECK_EQ(kInterpreterImplKind, kComputedGotoImplKind);
...
void EnterInterpreterFromInvoke(Thread *self, ArtMethod *method, Object *receiver, uint32_t *args,
                                JValue *result) {
    ...
    self->EndAssertNoThreadSuspension(old_cause);
    // Do this after populating the shadow frame in case EnsureInitialized
    // causes a GC.
    ...
    if (LIKELY(!method->IsNative())) {
        ...
        // We don't expect to be asked to interpret native code (which is
        // entered via a JNI compiler generated stub) except during testing and
        // image writing. Update args to be the args in the shadow frame since
        // the input ones could hold stale references pointers due to moving GC.
    ...
    while (shadow_frame != nullptr) {
        ...
        if (UNLIKELY(self->IsExceptionPending())) {
            ...
            uint32_t found_dex_pc =
                FindNextInstructionFollowingException(self, *shadow_frame, dex_pc, instrumentation);
