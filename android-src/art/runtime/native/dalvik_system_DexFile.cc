...
// fart start
// {{{
// Define the function at http://androidxref.com/6.0.1_r10/xref/art/runtime/art_method.cc
extern "C" void myfartInvoke(ArtMethod *artmethod);
// }}}
// fart end

static std::unique_ptr<std::vector<const DexFile *>> ConvertJavaArrayToNative(JNIEnv *env,
                                                                              jobject arrayObject) {
    ...
    jlong *long_data =
        env->GetLongArrayElements(reinterpret_cast<jlongArray>(array), &is_long_data_copied);
    if (env->ExceptionCheck() == JNI_TRUE) {
        return std::unique_ptr<std::vector<const DexFile *>>();
    }
...
// Unlike libcore's NullableScopedUtfChars, this will *not* throw
// NullPointerException if passed a null jstring. The correct idiom is:
//
...
//   // ... use name.c_str()
//
// TODO: rewrite to get rid of this, or change ScopedUtfChars to offer this
// option.
class NullableScopedUtfChars {
  public:
    ...
    size_t size() const { return strlen(mUtfChars); }

    // Element access.
    const char &operator[](size_t n) const { return mUtfChars[n]; }
...
static jobject DexFile_openDexFileNative(JNIEnv *env, jclass, jstring javaSourceName,
    ...
    if (!dex_files.empty()) {
        ...
        // The most important message is at the end. So set up nesting by going
        // forward, which will wrap the existing exception as a cause for the
        // following one.
    ...
    for (auto &dex_file : *dex_files) {
        ...
        if (dex_class_def != nullptr) {
            ...
            mirror::Class *result = class_linker->DefineClass(
                soa.Self(), descriptor.c_str(), hash, class_loader, *dex_file, *dex_class_def);
            if (result != nullptr) {
                VLOG(class_linker) << "DexFile_defineClassNative returning " << result << " for "
                                   << class_name.c_str();
...
// Note: this can be an expensive call, as we sort out duplicates in MultiDex
// files.
static jobjectArray DexFile_getClassNameList(JNIEnv *env, jclass, jobject cookie) {
    ...
    // Push all class descriptors into a set. Use set instead of unordered_set
    // as we want to retrieve all in the end.
    ...
    jobjectArray result =
        env->NewObjectArray(descriptors.size(), WellKnownClasses::java_lang_String, nullptr);
    if (result != nullptr) {
...
static void DexFile_dumpMethodCode(JNIEnv *env, jclass, jobject method) {
    ...
    if (method != nullptr) {
        // ArtMethod *ArtMethod::
        //   FromReflectedMethod(const ScopedObjectAccessAlreadyRunnable & soa,
        //   jobject jlr_method)
...
static jint GetDexOptNeeded(JNIEnv *env, const char *filename, const char *pkgname,
                            const char *instruction_set, const jboolean defer) {
...
static jint DexFile_getDexOptNeeded(JNIEnv *env, jclass, jstring javaFilename, jstring javaPkgname,
                                    jstring javaInstructionSet, jboolean defer) {
    ...
    return GetDexOptNeeded(env, filename.c_str(), pkgname.c_str(), instruction_set.c_str(), defer);
}
...
static jboolean DexFile_isDexOptNeeded(JNIEnv *env, jclass, jstring javaFilename) {
    ...
    ScopedUtfChars filename(env, javaFilename);
    jint status = GetDexOptNeeded(env, filename.c_str(), nullptr /* pkgname */, instruction_set,
                                  false /* defer */);
...
static JNINativeMethod gMethods[] = {
    ...
    NATIVE_METHOD(DexFile, defineClassNative,
                  "(Ljava/lang/String;Ljava/lang/ClassLoader;Ljava/lang/"
                  "Object;)Ljava/lang/Class;"),
