...
class ArtMethod FINAL {
  public:
    ArtMethod()
        : access_flags_(0), dex_code_item_offset_(0), dex_method_index_(0), method_index_(0) {}
    ...
    bool IsOptimized(size_t pointer_size) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
        ...
        // (as in stack_map.h).
        return !IsNative() && GetEntryPointFromQuickCompiledCodePtrSize(pointer_size) != nullptr &&
    ...
    size_t GetVtableIndex() SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) { return GetMethodIndex(); }
    ...
    uint32_t GetCodeItemOffset() { return dex_code_item_offset_; }
    ...
    // return DexFile::kDexNoIndex. The name_and_signature_idx MUST refer to a MethodId with the
    // same name and signature in the other_dexfile, such as the method index used to resolve this
    // method in the other_dexfile.
    ...
    void
    SetEntryPointFromInterpreterPtrSize(EntryPointFromInterpreter *entry_point_from_interpreter,
                                        size_t pointer_size) {
    ...
    ALWAYS_INLINE const void *GetEntryPointFromQuickCompiledCodePtrSize(size_t pointer_size) {
        return GetEntryPoint<const void *>(EntryPointFromQuickCompiledCodeOffset(pointer_size),
                                           pointer_size);
    }
    ...
    ALWAYS_INLINE void
    SetEntryPointFromQuickCompiledCodePtrSize(const void *entry_point_from_quick_compiled_code,
                                              size_t pointer_size) {
    ...
    const uint8_t *GetMappingTable(size_t pointer_size) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_);
    const uint8_t *GetMappingTable(const void *code_pointer, size_t pointer_size)
        SHARED_LOCKS_REQUIRED(Locks::mutator_lock_);

    // Callers should wrap the uint8_t* in a VmapTable instance for convenient access.
    const uint8_t *GetVmapTable(size_t pointer_size) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_);
    ...
    const uint8_t *GetNativeGcMap(size_t pointer_size) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_);
    ...
    static MemberOffset EntryPointFromInterpreterOffset(size_t pointer_size) {
        return MemberOffset(PtrSizedFieldsOffset(pointer_size) +
                            OFFSETOF_MEMBER(PtrSizedFields, entry_point_from_interpreter_) /
                                sizeof(void *) * pointer_size);
    }

    static MemberOffset EntryPointFromJniOffset(size_t pointer_size) {
        return MemberOffset(PtrSizedFieldsOffset(pointer_size) +
                            OFFSETOF_MEMBER(PtrSizedFields, entry_point_from_jni_) /
                                sizeof(void *) * pointer_size);
    }

    static MemberOffset EntryPointFromQuickCompiledCodeOffset(size_t pointer_size) {
        return MemberOffset(PtrSizedFieldsOffset(pointer_size) +
                            OFFSETOF_MEMBER(PtrSizedFields, entry_point_from_quick_compiled_code_) /
                                sizeof(void *) * pointer_size);
    }

    void *GetEntryPointFromJni() { return GetEntryPointFromJniPtrSize(sizeof(void *)); }
    ...
    // May cause thread suspension due to GetClassFromTypeIdx calling ResolveType this caused a
    // large number of bugs at call sites.
  ...
  protected:
    ...
    // the declaringClass.directMethods, for virtual methods the vtable and for interface methods
    // the ifTable.
    ...
    struct PACKED(4) PtrSizedFields {
        ...
        // Pointer to JNI function registered to this method, or a function to resolve the JNI
        // function.
  ...
  private:
    ...
    static bool PcIsWithinQuickCode(uintptr_t code, uintptr_t pc) {
        ...
        return code <= pc &&
               pc <= code +
                         GetCodeSize(EntryPointToCodePointer(reinterpret_cast<const void *>(code)));
    }
