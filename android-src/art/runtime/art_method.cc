        ...
        if (LIKELY(code_item != nullptr)) {
            ...
            if (code_item->tries_size_ > 0) {
                const uint8_t *handler_data =
                    (const uint8_t *)(DexFile::GetTryItems(*code_item, code_item->tries_size_));
            ...
            if (fp2 > 0) {
                ...
                sprintf(dexfilepath,
                        "{name:%s,method_idx:%d,offset:%d,code_item_len:%d,ins:", methodname,
...
ArtMethod *ArtMethod::FindOverriddenMethod(size_t pointer_size) {
    ...
    // Did this method override a super class method? If so load the result from
    // the super class' vtable
    if (super_class->HasVTable() && method_index < super_class->GetVTableLength()) {
        ...
        if (IsProxyMethod()) {
            ...
            CHECK_EQ(result, Runtime::Current()->GetClassLinker()->FindMethodForProxy(
    ...
    DCHECK(result == nullptr ||
           HasSameNameAndSignature(GetInterfaceMethodIfProxy(sizeof(void *)),
...
uint32_t ArtMethod::ToDexPc(const uintptr_t pc, bool abort_on_failure) {
    ...
    if (IsOptimized(sizeof(void *))) {
        ...
        MappingTable table(
            entry_point != nullptr
                ? GetMappingTable(EntryPointToCodePointer(entry_point), sizeof(void *))
                : nullptr);
        if (table.TotalSize() == 0) {
            // NOTE: Special methods (see Mir2Lir::GenSpecialCase()) have an
            // empty mapping but they have no suspend checks and, consequently,
            // we never call ToDexPc() for them.
    ...
    if (abort_on_failure) {
        LOG(FATAL) << "Failed to find Dex offset for PC offset "
                   << reinterpret_cast<void *>(sought_offset) << "(PC "
                   << reinterpret_cast<void *>(pc) << ", entry_point=" << entry_point
                   << " current entry_point=" << GetQuickOatEntryPoint(sizeof(void *)) << ") in "
                   << PrettyMethod(this);
    }
    return DexFile::kDexNoIndex;
}
...
uint32_t ArtMethod::FindCatchBlock(Handle<mirror::Class> exception_type, uint32_t dex_pc,
    ...
    for (CatchHandlerIterator it(*code_item, dex_pc); it.HasNext(); it.Next()) {
        ...
        if (UNLIKELY(iter_exception_type == nullptr)) {
            // Now have a NoClassDefFoundError as exception. Ignore in case the
            // exception class was removed by a pro-guard like tool. Note: this
            // is not RI behavior. RI would have failed when loading the class.
            self->ClearException();
            // Delete any long jump context as this routine is called during a
            // stack walk which will release its in use context at the end.
    ...
    if (found_dex_pc != DexFile::kDexNoIndex) {
        const Instruction *first_catch_instr = Instruction::At(&code_item->insns_[found_dex_pc]);
...
void ArtMethod::AssertPcIsWithinQuickCode(uintptr_t pc) {
    ...
    CHECK(PcIsWithinQuickCode(reinterpret_cast<uintptr_t>(code), pc))
        ...
        << " size=" << GetCodeSize(EntryPointToCodePointer(reinterpret_cast<const void *>(code)));
}
...
const void *ArtMethod::GetQuickOatEntryPoint(size_t pointer_size) {
    ...
    // quick-to-interpreter-bridge (but not the trampoline) for non-native
    // methods.
    if (class_linker->IsQuickToInterpreterBridge(code) ||
...
void ArtMethod::Invoke(Thread *self, uint32_t *args, uint32_t args_size, JValue *result,
    ...
    // If the runtime is not yet started or it is required by the debugger, then
    // perform the Invocation by the interpreter.
    if (UNLIKELY(!runtime->IsStarted() || Dbg::IsForcedInterpreterNeededForCalling(self, this))) {
        ...
        if (LIKELY(have_quick_code)) {
            ...
            // Ensure that we won't be accidentally calling quick compiled code
            // when -Xint.
            ...
            if (UNLIKELY(self->GetException() == Thread::GetDeoptimizationException())) {
                ...
                // exception was thrown to force the activations to be removed
                // from the stack. Continue execution in the interpreter.
                self->ClearException();
...
// Counts the number of references in the parameter list of the corresponding
// method. Note: Thus does _not_ include "this" for non-static methods.
...
QuickMethodFrameInfo ArtMethod::GetQuickFrameInfo() {
    ...
    // This goes before IsProxyMethod since runtime methods have a null
    // declaring class.
    if (UNLIKELY(IsRuntimeMethod())) {
        return runtime->GetRuntimeMethodFrameInfo(this);
    }
    // For Proxy method we add special handling for the direct method case
    // (there is only one direct method - constructor). Direct method is cloned
    // from original java.lang.reflect.Proxy class together with code and as a
    // result it is executed as usual quick compiled method without any stubs.
    // So the frame info should be returned as it is a quick method not a stub.
    // However, if instrumentation stubs are installed, the
    // instrumentation->GetQuickCodeFor() returns the artQuickProxyInvokeHandler
    // instead of an oat code pointer, thus we have to add a special case here.
    ...
    // native method indicating the generic JNI, or the
    // quick-to-interpreter-bridge (but not the trampoline) for non-native
    // methods. And we really shouldn't see a failure for non-native methods
    // here.
    ...
    if (class_linker->IsQuickGenericJniStub(entry_point)) {
        ...
        QuickMethodFrameInfo callee_info =
            runtime->GetCalleeSaveMethodFrameInfo(Runtime::kRefsAndArgs);

        // Callee saves + handle scope + method ref + alignment
        // Note: -sizeof(void*) since callee-save frame stores a whole method
        // pointer.
        size_t frame_size = RoundUp(callee_info.FrameSizeInBytes() - sizeof(void *) +
                                        sizeof(ArtMethod *) + scope_size,
                                    kStackAlignment);
        return QuickMethodFrameInfo(frame_size, callee_info.CoreSpillMask(),
