...
list1 = [
    'fmt10t', 'fmt10x', 'fmt11n', 'fmt11x', 'fmt12x', 'fmt20t', 'fmt21c', 'fmt21h', 'fmt21s',
    'fmt21t', 'fmt22b', 'fmt22c', 'fmt22s', 'fmt22t', 'fmt22x', 'fmt23x', 'fmt30t', 'fmt31c',
    'fmt31i', 'fmt31t', 'fmt32x', 'fmt35c', 'fmt3rc', 'fmt51l'
]
...
def parse_FMT11N(buffer, dex_object, pc_point, offset):
    return (
        dex_decode[ord(buffer[0])][4], dex_decode[ord(buffer[0])][1],
        "v%d" % (ord(buffer[1]) & 0xf), "%d" % ((ord(buffer[1]) >> 4) & 0xf)
    )
...
def parse_FMT12X(buffer, dex_object, pc_point, offset):
    return (
        dex_decode[ord(buffer[0])][4], dex_decode[ord(buffer[0])][1],
        "v%d" % (ord(buffer[1]) & 0x0f), "v%d" % ((ord(buffer[1]) >> 4) & 0xf)
    )
...
def parse_FMT21C(buffer, dex_object, pc_point, offset):
    ...
    return (
        dex_decode[ord(buffer[0])][4], dex_decode[ord(buffer[0])][1], "v%d" % ord(buffer[1]), arg1
    )
...
def parse_FMT22B(buffer, dex_object, pc_point, offset):
    ...
    return (
        dex_decode[ord(buffer[0])][4], dex_decode[ord(buffer[0])][1], "v%d" % ord(buffer[1]),
        "v%d" % bb, "%d" % cc
    )
...
def parse_FMT22C(buffer, dex_object, pc_point, offset):
    ...
    if ord(buffer[0]) == 0x20 or ord(buffer[0]) == 0x23:
        ...
        prefix = "field@%s  //%s" % (
            dex_object.getfieldname(cccc), dex_object.getfieldfullname(cccc)
        )

    bb = ord(buffer[1]) >> 4
    return (
        dex_decode[ord(buffer[0])][4], dex_decode[ord(buffer[0])][1],
        "v%d" % (ord(buffer[1]) & 0xf), "v%d" % ((ord(buffer[1]) >> 4) & 0xf), "%s" % prefix
    )
...
def parse_FMT22S(buffer, dex_object, pc_point, offset):
    ...
    return (
        dex_decode[ord(buffer[0])][4], dex_decode[ord(buffer[0])][1],
        "v%d" % (ord(buffer[1]) & 0xf), "v%d" % ((ord(buffer[1]) >> 4) & 0xf), "%d" % cccc
    )
...
def parse_FMT22T(buffer, dex_object, pc_point, offset):
    ...
    return (
        dex_decode[ord(buffer[0])][4], dex_decode[ord(buffer[0]
                                                     )][1], "v%d" % (ord(buffer[1]) & 0xf),
...
def parse_FMT30T(buffer, dex_object, pc_point, offset):
    ...
    return dex_decode[ord(buffer[0])][4], dex_decode[ord(buffer[0])][1], "+%x" % (
        aaaaaaaa + offset
    )
...
def parse_FMT32X(buffer, dex_object, pc_point, offset):
    ...
    return (
        dex_decode[ord(buffer[0])][4], dex_decode[ord(buffer[0])][1], "v%d" % aaaa, "v%d" % bbbb
    )
...
def parse_FMT35C(buffer, dex_object, pc_point, offset):
    ...
    if ord(buffer[0]) == 0x24:
        ...
        prefix = "meth@%s  //%s" % (
            dex_object.getmethodname(bbbb), dex_object.getmethodfullname(bbbb, True)
        )
        pass
    if A == 5:
        return (
            dex_decode[ord(buffer[0])][4], dex_decode[ord(buffer[0])][1], "v%d" % C, "v%d" % D,
            "v%d" % E, "v%d" % F, "v%d" % G, "%s" % (prefix)
        )
    elif A == 4:
        return (
            dex_decode[ord(buffer[0])][4], dex_decode[ord(buffer[0])][1], "v%d" % C, "v%d" % D,
            "v%d" % E, "v%d" % F, "%s" % (prefix)
        )
    elif A == 3:
        return (
            dex_decode[ord(buffer[0])][4], dex_decode[ord(buffer[0])][1], "v%d" % C, "v%d" % D,
        ...
        return (
            dex_decode[ord(buffer[0])][4], dex_decode[ord(buffer[0])][1], "v%d" % C,
            "%s" % (prefix)
        )
    elif A == 0:
...
func_point = [
    parse_FMT10T, parse_FMT10X, parse_FMT11N, parse_FMT11X, parse_FMT12X, parse_FMT20T,
    parse_FMT21C, parse_FMT21H, parse_FMT21S, parse_FMT21T, parse_FMT22B, parse_FMT22C,
    parse_FMT22S, parse_FMT22T, parse_FMT22X, parse_FMT23X, parse_FMT30T, parse_FMT31C,
    parse_FMT31I, parse_FMT31T, parse_FMT32X, parse_FMT35C, parse_FMT3RC, parse_FMT51L
]
...
def parseinsfile():
    ...
    insarray = re.findall(
        r"{name:(.*?),method_idx:(.*?),offset:(.*?),code_item_len:(.*?),ins:(.*?)}", content
    )  #(.*?)最短匹配
...
class method_code:
    ...
    def printf(self, dex_object, prefix=""):
        print "%s%-20s:%08x:%10d" % (
            prefix, "registers_size", self.registers_size, self.registers_size
        )
        print "%s%-20s:%08x:%10d" % (prefix, "insns_size", self.insns_size, self.insns_size)
        print "%s%-20s:%08x:%10d" % (
            prefix, "debug_info_off", self.debug_info_off, self.debug_info_off
        )
        ...
        parse_instruction(
            dex_object.m_content[self.insns:self.insns + self.insns_size * 2], self.insns,
            dex_object
        )
    ...
    def __init__(self, dex_obj, content):
        ...
        if self.tries_size == 0:
            ...
            for i in range(0, self.tries_size):
                temptryitem = tryitem(
                    self.dex_obj, content, self.handlerlist_offset, offset + 8 * i
                )
    ...
    def printf(self, dex_object, prefix=""):
        ...
        if self.tries_size > 0:
            for i in range(0, self.tries_size):
                ...
                print "%s%-20s:%08x:%10d" % (
                    prefix, "try[" + str(i) + "] ins start:", tryitem.start_addr,
                    tryitem.start_addr
                )
                print "%s%-20s:%08x:%10d" % (
                ...
                for j in range(len(tryitem.handlerlist)):
                    ...
                    print "%s%-20s:%s" % (
                        prefix, "try[" + str(i) + "]:handler[" + str(j) + "] exception type:",
                        temphandler.type_str
                    )
                    print "%s%-20s:%08x:%10d" % (
                        prefix, "try[" + str(i) + "]:handler[" + str(j) + "] ins start:",
                        temphandler.ins_start, temphandler.ins_start
                    )
        # print "%s%-20s:%08x:%10d" % (prefix, "insnsoffset", self.insns, self.insns)
        # print "%s%-20s:%08x:%10d" % (prefix, "triesoffset", self.tries, self.tries)
        parse_instruction(
            self.content[self.insns:self.insns + self.insns_size * 2], self.insns, dex_object
        )
...
class dex_class:
    ...
    def __init__(self, dex_object, classid):
        ...
        if self.interfacesOff != 0:
            self.interfacesSize, = struct.unpack_from(
                "I", dex_object.m_content, self.interfacesOff
            )
        ...
        for i in xrange(0, self.numStaticFields):
            ...
            if self.staticValuesOff:
                ...
                if staticoffset == -1:
                    str1 += "0;\n"
                    continue
                size, str2 = parse_encoded_value1(
                    dex_object, dex_object.m_content[self.staticValuesOff + staticoffset:]
                )
                str1 += str2
            str1 += ";\n"
        field_idx = 0
    ...
    def printf(self, dex_object):
        ...
        print "%-20s:%08x:%10d  %s" % (
            "superClass", self.superClass, self.superClass,
        ...
        print "%-20s:%08x:%10d" % (
            "numInstanceFields", self.numInstanceFields, self.numInstanceFields
        )
        print "%-20s:%08x:%10d" % (
            "numDirectMethods", self.numDirectMethods, self.numDirectMethods
        )
        print "%-20s:%08x:%10d" % (
            "numVirtualMethods", self.numVirtualMethods, self.numVirtualMethods
        )
        ...
        print "%-20s:%08x:%10d  %s" % (
            "sourceFileIdx", self.sourceFileIdx, self.sourceFileIdx,
            dex_object.getstringbyid(self.sourceFileIdx)
        )
        offset = self.classDataOff
        n, tmp = get_uleb128(dex_object.m_content[offset:offset + 5])
        offset += n
        ...
        for i in xrange(0, self.numDirectMethods):
            ...
            method_idx += method_idx_diff
            if code_off != 0:
                methodname = dex_object.getmethodfullname(method_idx,
                                                          True).replace("::",
                                                                        ".").replace(" ", "")
                method = None
                try:
        ...
        for i in xrange(0, self.numVirtualMethods):
            ...
            if code_off != 0:
                ...
                try:
                    ...
                    pass
                if method != None:
                    print "\nVirtualMethod:" + dex_object.getmethodfullname(
                        method_idx, True
                    ) + "\n"
                    try:
                        print "before repire method+++++++++++++++++++++++++++++++++++\n"
        ...
        if self.annotationsOff != 0:
            ...
            if self.fields_size:
                for i in xrange(0, self.fields_size):
                    field_idx, annotations_off, = struct.unpack_from(
                        "2I", dex_object.m_content, offset
                    )
            ...
            if self.annotated_methods_size:
                ...
                for i in xrange(0, self.annotated_methods_size):
                    method_idx, annotations_off, = struct.unpack_from(
                        "2I", dex_object.m_content, offset
                    )
                    offset += struct.calcsize("2I")
...
class annotations_directory_item:
    ...
    def __init__(self, content, dex_object):
        ...
        if self.annotated_methods_size:
            ...
            self.m_methods_list.fromstring(
                content[offset:offset + 8 * self.annotated_methods_size]
            )
        offset = offset + 4 * self.annotated_methods_size
        for i in xrange(0, annotated_methods_size):
            self.m_parameters_list = array.array("L")
            self.m_parameters_list.fromstring(
                content[offset:offset + 8 * self.annotated_parameters_size]
            )
    ...
    while offset < totalsize:
        ...
        if bytecode == 0:
            ...
            print "v%d %s %s  START_LOCAL" % (
                register_num, lex_object.gettypenamebyid(type_idx),
                lex_object.getstringbyid(name_idx)
            )
        elif bytecode == 4:
            n, register_num = get_uleb128(lex_object.m_content[offset:offset + 5])
            offset += n
            ...
            print "line=%d  pc=%x  adjusted_opcode=%d  pc+ %d  line+%d" % (
                current_line, current_pc, adjusted_opcode, (adjusted_opcode / 15),
                (adjusted_opcode % 15) - 4
            )
    print "===parse_debug_info====offset = %08x$" % offset
...
def get_encoded_value(content):
    ...
    VALUE_BOOLEAN = 0x1f
    type_enum = [
        0x0, 0x2, 0x3, 0x4, 0x6, 0x10, 0x11, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f
    ]
...
class dex_parser:
    ...
    def get_access_flags1(self, flags):
        val = {1: "public", 2: "private", 4: "protected"}
        value = ""
        i = 0
        for key in val:
            if key & flags:
